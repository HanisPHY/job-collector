...
### 0.4 Prompt 的必要要素:两个家族

公开的 prompt 框架(CO-STAR / CRAFT / RTF,以及各家的 best practice)基本都在回答
同一个问题:**"agent 需要知道什么"**。这类要素是必要的,但它们主要为**单轮生成**设计。

第 0.3 节那五类坑,几乎全部落在另一个家族:
**"prompt 没写到的地方,agent 自己拿主意时该按什么规矩"**。

**任务越自主、链路越长,第二个家族越主导结果。**

#### A. 信息类 —— agent 需要知道什么

| 要素 | 说明 | 不写会怎样 |
|---|---|---|
| **角色 + 反角色** | 你是谁,以及**你不该做什么** | 角色越界。反角色往往比角色更重要("不写代码""不评价产品价值") |
| **原始需求,逐字不可改写** | 唯一不能被摘要的字段 | 多轮之后 agent 记住的是自己的复述,不是你的原话,需求会缓慢漂移 |
| **事实 context** | 真实探查出来的 workflow / 业务逻辑 / 数据分布,**不是脑补的** | agent 用"通用场景下最合理的假设"填空 |
| **工具与环境** | 具体到解释器路径、环境变量、怎么跑 | "有 Python"不够;不给 `PYTHONIOENCODING` 就会踩 §0.3-C 那个编码崩溃 |
| **输出契约** | 结构化到能程序化判断(verdict + issues + severity) | 自由文本无法判断循环是否该终止 |

> Anthropic 讲的 "right altitude" 就是这一类的核心矛盾:太具体会脆,太笼统会
> **假设共享上下文**——而"假设共享上下文"正是本项目最主要的失败源。

#### B. 决策类 —— prompt 说完之后按什么规矩办

| 要素 | 说明 | 对应 §0.3 |
|---|---|---|
| **范围边界** | 要设计的 / 不要重新设计的 / 明确不做(附是谁在什么时候否掉的) | A |
| **证据标准** | 推翻既有结论要拿什么当货币 + **明确授权 agent 反对你** | B |
| **验证环境** | 在什么环境验、哪几条必须真跑而不能只看代码 | C |
| **完成定义 + 终止条件** | 逐条可判定的验收 + max_iterations | — |

**如果只能记三条**,挑这三个——因为它们是**只有你知道、agent 再聪明也推不出来**的:
事实 context、范围边界、证据标准。

其中**证据标准最容易被忽略**,因为它反直觉:你会以为"agent 讲得有道理"是好事,
但 LLM 为任何立场生成顺畅论证的能力接近无限,所以**"讲得通"几乎不携带信息量**。
不设证据标准,你收到的就是一堆无法证伪的合理性。

#### 边界会衰减,必须每轮重申

多个公开来源报告了 **constraint decay**:随着上下文累积,
早期声明的约束**影响力会持续下降**,因此要在**每个决策点重新注入约束**,
而不是开头声明一次就当它一直生效。

> 注:§0.3 的案例都是本项目实测的;constraint decay 这条来自公开资料,
> 但与本项目的实际做法吻合——上一轮 orchestrator 每一轮 SendMessage 都在重申边界
> (「这一轮只做减法」「不要再引入新机制」「只审增量」),当时是凭感觉,事后看是有原因的。

这条有一个直接的工程含义:
**把规则写进 §5 的 prompt 模板,比写进 §1 的设计原则更管用**——
模板每轮都会被填一次,原则只在开头被读一次。

---

### 0.5 交接文档自检清单

**交接文档**(如 `docs/req/*.md`)是给**未来某一轮对话**看的,长期存在、会被反复读。
它承载的是**事实和历史决策**,不承载角色和输出格式。

- [ ] **哪些已定、哪些待议**切开了吗?(单列两节,不要混在叙述里)
- [ ] 有没有一节写**明确不做什么**,以及**是谁在什么时候否掉的**?
- [ ] 规定了**推翻既有结论的证据标准**吗?有没有指出**现成的复现脚本在哪**?
- [ ] 每条约束都写了**为什么**吗?(只有约束没有理由,下一轮会当成风格偏好推翻掉)
- [ ] 文档里每个数字,标注了**在什么配置、什么时间测出来的**吗?
- [ ] 会过期的数字,有没有写清**该怎么重新测**?
- [ ] 环境信息完整吗?(解释器路径、环境变量、怎么跑测试)
- [ ] 每个"不能超过 X"的要求,背后有**机制**还是只有**目标**?
- [ ] 有没有哪条约束把**两件不同的事压成了一件**?

### 0.6 Prompt 自检清单

**Prompt** 是给**这一次调用**的,一次性、每轮重发。
它要从交接文档里**切片**,不是整本塞进去。

- [ ] 角色**和反角色**都写了吗?
- [ ] 原始需求是**逐字**给的,还是被我摘要过了?
- [ ] 给的是**这个 agent 需要的那一片** context,还是整个状态全量塞进去了?
- [ ] 工具和环境**具体**吗?(路径、环境变量,不是"你有 bash")
- [ ] 输出格式**能被程序判断**吗?
- [ ] **范围边界这一轮重申了吗?**(不能依赖它在上一轮已经读过——见上面的 constraint decay)
- [ ] **明确授权它反对我**了吗?
- [ ] 验收里哪几条**必须真跑**,写清楚了吗?
- [ ] 有终止条件吗?(这一轮做完什么算完)
- [ ] 多个 agent 并行时,写了**另一个 agent 的分工**、让它别重复吗?
...
# 范围边界(必填,见 §0.3-A)
> ⚠ 这一段**每一轮都要完整重发**,不能因为上一轮已经给过就省掉 ——
> 约束的影响力会随上下文累积而衰减(constraint decay,见 §0.4)。
